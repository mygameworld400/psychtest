...
import { matchAfterRules } from '../services/afterRules'
import { getAssessmentResult } from '../services/assessmentService'

const DIMENSION_LABELS = Object.fromEntries(afterQuestionBank.dimensions.map((d) => [d.key, d.label]))

export default function AfterResult() {
  ...
  const headlines = matchAfterRules(scoreMap)
  const shareUrl = `${location.origin}${location.pathname}#/after/result/${assessmentId}`

  return (
    <div className="page page-me-result">
      <h1>현재 이별 상태</h1>
      <p className="sub">
        지금 응답 기준으로 나타난 신호야. &quot;재결합 확률&quot; 같은 미래 예측이 아니라 지금의 경향이야.
      </p>
